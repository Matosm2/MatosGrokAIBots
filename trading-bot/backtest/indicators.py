...
def qqe(
    ...
    Formula (Livshin / Kivanc):
      rsi_raw = rsi(close, rsi_len)
      qqe_fast = ema(rsi_raw, sf)
      tr = abs(qqe_fast - qqe_fast[1])
      wilder_len = 2 * rsi_len - 1
      atr_rsi = rma(rma(tr, wilder_len), wilder_len)
      dar = atr_rsi * wt
      qqe_slow: classic ratchet trailing stop:
        new_long = qqe_fast - dar
        new_short = qqe_fast + dar
        if qqe_fast[1] > slow[1] (in long mode):
          slow = max(new_long, slow[1]) if qqe_fast > slow[1] else new_short
        else:
          slow = min(new_short, slow[1]) if qqe_fast < slow[1] else new_long

    Returns (qqe_fast, qqe_slow).
    != raw RSI 70/30 / != Schaff / != WaveTrend / != ConnorsRSI.
    """
    n = len(closes)
    fast_out: list[float | None] = [None] * n
    slow_out: list[float | None] = [None] * n
    if n == 0 or rsi_len <= 0 or sf <= 0 or wt <= 0.0:
    ...
    first_rsi = 0
    while first_rsi < n and rsi_vals[first_rsi] is None:
        first_rsi += 1
    if first_rsi >= n:
        return fast_out, slow_out

    # Compute EMA of RSI
    rsi_clean = [float(x) for x in rsi_vals[first_rsi:]]  # type: ignore[arg-type]
    fast_sub = ema(rsi_clean, sf)
    for j, f in enumerate(fast_sub):
        fast_out[first_rsi + j] = f
    ...
    for i in range(1, n):
        ...
        fp = fast_out[i - 1]
        if fi is not None and fp is not None:
            tr[i] = abs(fi - fp)

    wilder_len = 2 * rsi_len - 1
    # Wilder smooth twice
    atr_rsi_1 = rma(tr, wilder_len)
    atr_rsi = rma(atr_rsi_1, wilder_len)

    # Trail slow line
    # Find first index where fast_out and atr_rsi are both available
    first_valid = 0
    while first_valid < n and (fast_out[first_valid] is None or atr_rsi[first_valid] is None):
        first_valid += 1

    if first_valid >= n:
        return fast_out, slow_out

    # Initialize slow at first_valid
    init_fast = float(fast_out[first_valid])  # type: ignore[arg-type]
    slow_out[first_valid] = init_fast
    ...
    for i in range(first_valid + 1, n):
        ...
        if f is None or f_prev is None or s_prev is None or a is None:
            continue
        ...
        if f_prev > s_prev:
            # Was in long mode
            if f > s_prev:
                slow_out[i] = max(s_prev, new_long)
            else:
                slow_out[i] = new_short
        else:
            # Was in short mode
            if f < s_prev:
                slow_out[i] = min(s_prev, new_short)
            else:
                slow_out[i] = new_long
...
def mama_fama(
    ...
    Pure-Python implementation of Ehlers' Homodyne Discriminator pipeline to derive alpha:
      Phase from Hilbert Transform (I1, Q1), phase rate-of-change DeltaPhase,
      alpha = clamp(fast_limit / DeltaPhase, slow_limit, fast_limit)
      mama = alpha * price + (1 - alpha) * mama[1]
      fama = 0.5 * alpha * mama + (1 - 0.5 * alpha) * fama[1]

    != MESA-primary period / != Sinewave / != PMA / != FRAMA / != HMA.
    """
    n = len(prices)
    ...
    c0, c1, c2, c3 = 0.0962, 0.5769, -0.5769, -0.0962

    # Ring buffers / history
    # We can maintain history arrays
    smooth = [0.0] * n
    detrender = [0.0] * n
    i1 = [0.0] * n
    q1 = [0.0] * n
    ...
    for i in range(n):
        p = prices[i]
        if i >= 3:
            smooth[i] = (4.0 * p + 3.0 * prices[i - 1] + 2.0 * prices[i - 2] + prices[i - 3]) / 10.0
        else:
            smooth[i] = p

        gain = 0.075 * period + 0.54

        if i >= 6:
            # 7-tap Hilbert on smooth: taps 0, 2, 4, 6 (0 is i, 2 is i-2, etc.)
            q_val = (c0 * smooth[i] + c1 * smooth[i - 2] + c2 * smooth[i - 4] + c3 * smooth[i - 6]) * gain
            detrender[i] = q_val
        else:
            detrender[i] = 0.0

        if i >= 12:
            # 7-tap Hilbert on detrender
            i1[i] = detrender[i - 3]
            q1[i] = (c0 * detrender[i] + c1 * detrender[i - 2] + c2 * detrender[i - 4] + c3 * detrender[i - 6]) * gain
        else:
            i1[i] = 0.0
            q1[i] = 0.0

        if i >= 18:
            # 7-tap Hilbert on i1 and q1
            j_i[i] = (c0 * i1[i] + c1 * i1[i - 2] + c2 * i1[i - 4] + c3 * i1[i - 6]) * gain
            j_q[i] = (c0 * q1[i] + c1 * q1[i - 2] + c2 * q1[i - 4] + c3 * q1[i - 6]) * gain
        else:
            j_i[i] = 0.0
            j_q[i] = 0.0

        # Homodyne discriminator and period update after bar 18
        if i >= 18:
            i2_raw = i1[i] - j_q[i]
            q2_raw = q1[i] + j_i[i]

            i2 = 0.2 * i2_raw + 0.8 * i2_prev
            q2 = 0.2 * q2_raw + 0.8 * q2_prev

            re_raw = i2 * i2_prev + q2 * q2_prev
            im_raw = i2 * q2_prev - q2 * i2_prev

            i2_prev = i2
            q2_prev = q2

            re = 0.2 * re_raw + 0.8 * re_prev
            im = 0.2 * im_raw + 0.8 * im_prev
            ...
            calc_period = min(calc_period, 1.5 * period) if period > 0 else calc_period
            calc_period = max(calc_period, 0.67 * period) if period > 0 else calc_period
            calc_period = max(6.0, min(50.0, calc_period))

            period = 0.2 * calc_period + 0.8 * period
            smooth_period = 0.33 * period + 0.67 * smooth_period

        # Warmup complete around bar 22 (taps: 3 + 6 + 6 + 6 = 21 bars)
        if i >= 22:
            cur_i1 = i1[i]
            cur_q1 = q1[i]

            phase = math.atan(cur_q1 / cur_i1) * rad_to_deg if cur_i1 != 0.0 else 0.0
            delta_phase = max(1.0, prev_phase - phase)
            prev_phase = phase

            alpha = min(fast_limit, max(slow_limit, fast_limit / delta_phase))
            ...
            if not seeded:
                ...
                mama = alpha * p + (1.0 - alpha) * mama
                half_alpha = 0.5 * alpha
                fama = half_alpha * mama + (1.0 - half_alpha) * fama
...
def wilder_volatility_system(
    ...
    """Wilder Volatility System (ARC SAR).

    Formula (Wilder 1978 pp.23-26):
      atr = ta.atr(n)
      arc = factor * atr
      sarLong = ta.highest(close, n) - arc
      sarShort = ta.lowest(close, n) + arc

    Returns (sar_long, sar_short).
    != SuperTrend (HL2 ratchet) / != Chandelier (HH - ATR from entry) / != PSAR.
    """
    count = len(closes)
    sar_long: list[float | None] = [None] * count
    sar_short: list[float | None] = [None] * count
    if count == 0 or n <= 0 or factor <= 0.0:
        return sar_long, sar_short
...
def chande_kroll_stop(
    ...
    """Chande Kroll Stop (two-stage ATR corridor).
    ...
    != Chandelier (single HH - ATR) / != SuperTrend / != Wilder VS.
    """
    n = len(closes)
    stop_short: list[float | None] = [None] * n
    stop_long: list[float | None] = [None] * n
    if n == 0 or p <= 0 or x <= 0.0 or q <= 0:
    ...
    for i in range(p - 1, n):
        ...
        high_stop[i] = max(window_h) - x * a
        low_stop[i] = min(window_l) + x * a

    for i in range(p + q - 2, n):
        w_hs = [high_stop[k] for k in range(i - q + 1, i + 1) if high_stop[k] is not None]
        w_ls = [low_stop[k] for k in range(i - q + 1, i + 1) if low_stop[k] is not None]
        if len(w_hs) == q and len(w_ls) == q:
            stop_short[i] = max(w_hs)  # type: ignore[arg-type]
            stop_long[i] = min(w_ls)   # type: ignore[arg-type]
...
def guppy_cbl(
    ...
    """Guppy Count Back Line (CBL SAR flip).

    Formula (Daryl Guppy):
      From each bar i:
        cblLong: count back 3 successively lower lows from lows[i].
                 On long positions, cblLong ratchets up (never lowers).
        cblShort: count back 3 successively higher highs from highs[i].
                  On short positions, cblShort ratchets down (never raises).
      When close crosses under cblLong, the long trend is broken and cblLong resets.
      When close crosses over cblShort, the short trend is broken and cblShort resets.

    Returns (cbl_long, cbl_short).
    != Wilder VS (ARC = factor * ATR) / != Chande-Kroll (two-stage ATR) / != SuperTrend / != PSAR.
    """
    n = len(closes)
    cbl_long: list[float | None] = [None] * n
    cbl_short: list[float | None] = [None] * n
    if n == 0 or count_depth <= 0:
        return cbl_long, cbl_short

    cur_cbl_long: float | None = None
    cur_cbl_short: float | None = None

    for i in range(n):
        h = highs[i]
        l = lows[i]

        # Calculate candidate CBL long: scan left for 3 successively lower lows starting from lows[i]
        curr_low = l
        found_l = 1
        cand_long = curr_low
        for j in range(i - 1, -1, -1):
            if lows[j] < curr_low:
                curr_low = lows[j]
                found_l += 1
                if found_l >= count_depth:
                    cand_long = curr_low
                    break

        # Calculate candidate CBL short: scan left for 3 successively higher highs starting from highs[i]
        curr_high = h
        found_h = 1
        cand_short = curr_high
        for j in range(i - 1, -1, -1):
            if highs[j] > curr_high:
                curr_high = highs[j]
                found_h += 1
                if found_h >= count_depth:
                    cand_short = curr_high
                    break

        # Ratchet logic
        if cur_cbl_long is None:
            cur_cbl_long = cand_long
        else:
            cur_cbl_long = max(cur_cbl_long, cand_long)

        if cur_cbl_short is None:
            cur_cbl_short = cand_short
        else:
            cur_cbl_short = min(cur_cbl_short, cand_short)

        cbl_long[i] = cur_cbl_long
        cbl_short[i] = cur_cbl_short

        # Update ratchets for subsequent bars:
        # If close breaches cblLong, reset long ratchet to None to await fresh setup
        if closes[i] < cur_cbl_long:
            cur_cbl_long = None
        # If close breaches cblShort, reset short ratchet to None to await fresh setup
        if closes[i] > cur_cbl_short:
            cur_cbl_short = None

    return cbl_long, cbl_short


def kirshenbaum_bands(
    closes: list[float],
    ema_len: int = 20,
    reg_len: int = 20,
    k: float = 1.75,
    ...
    Formula:
      ...
      LinReg residual standard error over reg_len:
        fitted line: y_pred[j] = slope * j + intercept
        stderr = sqrt( sum( (close[j] - y_pred[j])^2 ) / reg_len )
      upper = mid + k * stderr
      lower = mid - k * stderr

    Returns (upper, mid, lower, stderr_series).
    Mid MUST be EMA (kill if LinReg mid -> SEB).
    != Bollinger Bands (stdev of price) / != SEB / != STARC.
    """
    n = len(closes)
    upper: list[float | None] = [None] * n
    lower: list[float | None] = [None] * n
    stderr_series: list[float | None] = [None] * n

    mid = ema(closes, ema_len)
    if n < reg_len or reg_len <= 1:
        return upper, mid, lower, stderr_series

    # Precompute LinReg constants for x = 0..reg_len-1
    # sum_x = (L-1)*L/2
    # sum_x2 = (L-1)*L*(2*L-1)/6
    # denom = L * sum_x2 - sum_x^2 = L^2*(L^2-1)/12
    sum_x = (reg_len - 1) * reg_len / 2.0
    denom = reg_len * ((reg_len - 1) * reg_len * (2 * reg_len - 1) / 6.0) - (sum_x * sum_x)

    for i in range(reg_len - 1, n):
        m = mid[i]
        if m is None:
            continue
        window_y = closes[i - reg_len + 1 : i + 1]
        sum_y = sum(window_y)
        sum_xy = sum(x * y for x, y in enumerate(window_y))

        slope = (reg_len * sum_xy - sum_x * sum_y) / denom if denom != 0.0 else 0.0
        intercept = (sum_y - slope * sum_x) / reg_len

        # Residual sum of squares
        sse = sum((y - (slope * x + intercept)) ** 2 for x, y in enumerate(window_y))
        # Standard error of residuals
        s_err = math.sqrt(max(0.0, sse / reg_len))
        stderr_series[i] = s_err
        upper[i] = m + k * s_err
        lower[i] = m - k * s_err
...
def imi(
    ...
    """Intraday Momentum Index (Tushar Chande).

    Formula:
      up = max(close - open, 0)
      dn = max(open - close, 0)
      imi = 100 * sum(up, n) / (sum(up, n) + sum(dn, n))
      guard: if denom == 0 -> 50.0
    ...
    ups = [max(closes[i] - opens[i], 0.0) for i in range(count)]
    dns = [max(opens[i] - closes[i], 0.0) for i in range(count)]
...
def williams_ad(
    ...
    """Williams Accumulation / Distribution (WAD) and SMA(WAD, m).
...
def katsanos_stiffness(
    ...
    """Katsanos Stiffness indicator (TASC Nov 2018).

    Reference:
      Markos Katsanos, "Stiffness Indicator", Technical Analysis of Stocks & Commodities Nov 2018.
      mkatsanos.com; Thinkorswim; Traders' Tips Nov 2018.

    Formulation:
      ma = SMA(Close, MAB)
      sd = StDev(Close, MAB) (sample stdev, or population stdev)
      ma2 = ma - NSTD * sd
    ...
    Returns:
      (stiffness_series, stif_series, ma2_series)
    != FVE (volume MF) / != VFI / != CHOP / != ADX / != Qstick.
    """
    count = len(closes)
    stiffness_series: list[float | None] = [None] * count
    stif_series: list[float | None] = [None] * count
    ma2_series: list[float | None] = [None] * count

    if count == 0 or mab <= 0 or period <= 0 or sm <= 0:
        return stiffness_series, stif_series, ma2_series

    # 1. Compute SMA and StDev of Close over MAB
    ma = sma(closes, mab)
    sd: list[float | None] = [None] * count
    for i in range(mab - 1, count):
        m = ma[i]
        if m is not None:
            win = closes[i - mab + 1 : i + 1]
            var = sum((x - m) ** 2 for x in win) / mab
            sd[i] = var ** 0.5

    # 2. ma2 = ma - nstd * sd
    for i in range(count):
        if ma[i] is not None and sd[i] is not None:
            ma2_series[i] = ma[i] - nstd * sd[i]

    # 3. stif = 100.0 * Sum(Close > ma2, Period) / Period
    # Note: Close > ma2 requires ma2 to be defined
    above = [0.0] * count
    for i in range(count):
        m2 = ma2_series[i]
        if m2 is not None and closes[i] > m2:
            above[i] = 1.0

    stif_raw: list[float | None] = [None] * count
    sum_above = 0.0
    for i in range(count):
        sum_above += above[i]
        if i >= period:
            sum_above -= above[i - period]
        # Valid when we have at least mab - 1 + period bars of valid ma2
        warmup = mab - 1 + period - 1
        if i >= warmup:
            val = 100.0 * sum_above / period
            stif_raw[i] = val
            stif_series[i] = val

    # 4. stiffness = EMA(stif, SM)
    valid_indices = [i for i, v in enumerate(stif_raw) if v is not None]
    if valid_indices:
        stif_clean = [stif_raw[i] for i in valid_indices]  # type: ignore[misc]
        ema_clean = ema(stif_clean, sm)
        for idx, ema_v in zip(valid_indices, ema_clean):
            stiffness_series[idx] = ema_v
...
def cpr_range(
    ...
    """Central Pivot Range (CPR) for same-timeframe rolling N lookback (no request.security).
    ...
    Returns:
      (tc_series, p_series, bc_series, width_series)
    != Classic Floor R1/S1 (R1=2P-L / S1=2P-H) / != Camarilla / != Woodie / != HHLL BOS.
    """
    count = len(closes)
    tc_series: list[float | None] = [None] * count
    p_series: list[float | None] = [None] * count
    bc_series: list[float | None] = [None] * count
    width_series: list[float | None] = [None] * count

    if count == 0 or n <= 0 or len(highs) != count or len(lows) != count:
        return tc_series, p_series, bc_series, width_series

    # For bar i, we need the rolling highest/lowest over N bars ending at bar i-1.
    # That requires i >= n.
    for i in range(n, count):
        ph = max(highs[i - n : i])
        pl = min(lows[i - n : i])
...
def varadi_dvs(
    ...
    """Varadi DVS (Directional Volatility Stretch) oscillator.

    Reference:
      David Varadi, CSS Analytics (July 31, 2009), "DVI Part 1: The Stretch Indicator (DVS)".
      Daily score DS = +1 up / -1 down / 0 flat
      sumDS = sum(DS, sum_len)
      raw = 0.5 * (sumDS + sumDS[1])
      dvs = 100.0 * percentrank(raw, rank_len)
        where percentrank counts fraction of window values <= current value:
        c_le = sum(1 for w in window if w <= curr) / rank_len

    Returns:
      (dvs_series, raw_series)
    != DVI (composite magnitude+stretch) / != DV2 (close/(high+low)) / != PSY.
    """
    count = len(closes)
    dvs_series: list[float | None] = [None] * count
    raw_series: list[float | None] = [None] * count

    if count == 0 or sum_len <= 0 or rank_len <= 0:
        return dvs_series, raw_series

    # 1. Compute directional score ds: +1 if C > C[1], -1 if C < C[1], 0 if C == C[1]
    ds = [0.0] * count
    for i in range(1, count):
        if closes[i] > closes[i - 1]:
            ds[i] = 1.0
    ...
    # 2. sumDS = sum(ds, sum_len)
    sum_ds = [0.0] * count
    curr_sum = 0.0
    for i in range(count):
        curr_sum += ds[i]
        if i >= sum_len:
            curr_sum -= ds[i - sum_len]
        if i >= sum_len - 1:
            sum_ds[i] = curr_sum

    # 3. raw = 0.5 * (sumDS + sumDS[1])
    # valid for i >= sum_len
    for i in range(sum_len, count):
        raw_series[i] = 0.5 * (sum_ds[i] + sum_ds[i - 1])

    # 4. dvs = 100.0 * percentrank(raw, rank_len)
    # Need rank_len bars of valid raw values: starts at sum_len + rank_len - 1
    start_idx = sum_len + rank_len - 1
    for i in range(start_idx, count):
        # Window of rank_len raw values ending at index i
        window = [raw_series[j] for j in range(i - rank_len + 1, i + 1)]  # type: ignore[misc]
        curr = raw_series[i]
        c_le = sum(1 for w in window if w is not None and w <= curr)
        dvs_series[i] = 100.0 * (c_le / rank_len)

    return dvs_series, raw_series


def historical_volatility_ratio(
    closes: list[float],
    short_len: int = 10,
    long_len: int = 100,
) -> tuple[list[float | None], list[float | None], list[float | None]]:
    """Historical Volatility Ratio (HVR) based on close-to-close log returns.

    Reference:
      TC2000 / Sierra Chart / Investopedia Historical Volatility.
      lr = ln(Close / Close[1])
      hvS = stdev(lr, short_len)
      hvL = stdev(lr, long_len)
      hvr = hvS / hvL (annualization factors cancel)

    Returns:
      (hvr_series, hv_short_series, hv_long_series)
    != Schwager VR (Japanese TR / Donchian) / != Parkinson (ln(H/L) RMS) /
    != Chaikin Volatility (% change EMA(H-L)) / != ATR ratio.
    """
    import math

    count = len(closes)
    hvr_series: list[float | None] = [None] * count
    hvs_series: list[float | None] = [None] * count
    hvl_series: list[float | None] = [None] * count

    if count < 2 or short_len <= 0 or long_len <= 0:
        return hvr_series, hvs_series, hvl_series

    # 1. Log returns: lr[i] = ln(C[i] / C[i-1])
    lr = [0.0] * count
    for i in range(1, count):
        if closes[i - 1] > 0 and closes[i] > 0:
            lr[i] = math.log(closes[i] / closes[i - 1])
        else:
            lr[i] = 0.0

    # 2. hvS: sample/population stdev of lr over short_len
    # Bars 1..count-1 have valid log returns
    for i in range(short_len, count):
        win = lr[i - short_len + 1 : i + 1]
        mean = sum(win) / short_len
        var = sum((x - mean) ** 2 for x in win) / short_len
        hvs_series[i] = var ** 0.5

    # 3. hvL: stdev of lr over long_len
    for i in range(long_len, count):
        win = lr[i - long_len + 1 : i + 1]
        mean = sum(win) / long_len
        var = sum((x - mean) ** 2 for x in win) / long_len
        hvl_series[i] = var ** 0.5

    # 4. hvr = hvS / hvL
    warmup = max(short_len, long_len)
...
def apirine_sdo(
    ...
    count = len(closes)
    out: list[float | None] = [None] * count
    if count == 0 or n <= 0 or lb <= 0 or pds <= 0:
        return out

    dist = [0.0] * count
    for i in range(count):
        if i >= n:
            dist[i] = abs(closes[i] - closes[i - n])
        else:
            dist[i] = 0.0
    ...
    for i in range(count):
        if i < n + lb - 1:
            signed[i] = 0.0
            continue
        window = dist[i - lb + 1 : i + 1]
        llv = min(window)
        hhv = max(window)
        rng = hhv - llv
        raw = (dist[i] - llv) / rng * 100.0 if rng > 1e-12 else 0.0
        c_cur = closes[i]
        c_prev = closes[i - n]
        if c_cur > c_prev:
            signed[i] = raw
    ...
    valid_signed = [
        signed[i] if i >= (n + lb - 1) else None
        for i in range(count)
    ]
    sdo = ema_of_optional(valid_signed, pds)
    return sdo
...
def ehlers_madh(
    ...
    """Ehlers Moving Average Difference Hann (TASC Nov 2021).

    Hann weights: w(i) = 1 - cos(2*pi*i / (L + 1)) for i = 1..L
    Filt1 = sum(w(i) * Close[i-1]) / sum(w(i)) over ShortLength
    LongLength = floor(ShortLength + DominantCycle / 2)
    Filt2 = sum(w(i) * Close[i-1]) / sum(w(i)) over LongLength
    MADH = 100 * (Filt1 - Filt2) / Filt2
    """
    count = len(closes)
    madh: list[float | None] = [None] * count
    filt1_out: list[float | None] = [None] * count
    filt2_out: list[float | None] = [None] * count

    long_length = int(short_length + dominant_cycle / 2)
    if count == 0 or short_length <= 0 or long_length <= 0:
        return madh, filt1_out, filt2_out

    # Precalculate Hann weights for short_length: i=1..L
    w1 = [1.0 - math.cos(2.0 * math.pi * i / (short_length + 1)) for i in range(1, short_length + 1)]
    sum_w1 = sum(w1)

    # Precalculate Hann weights for long_length: i=1..L
    w2 = [1.0 - math.cos(2.0 * math.pi * i / (long_length + 1)) for i in range(1, long_length + 1)]
    sum_w2 = sum(w2)

    for i in range(count):
        if i >= short_length - 1 and sum_w1 > 1e-12:
            f1 = sum(w1[k] * closes[i - k] for k in range(short_length)) / sum_w1
            filt1_out[i] = f1
        if i >= long_length - 1 and sum_w2 > 1e-12:
            f2 = sum(w2[k] * closes[i - k] for k in range(long_length)) / sum_w2
            filt2_out[i] = f2

        if filt1_out[i] is not None and filt2_out[i] is not None:
            f1_val = filt1_out[i]
            f2_val = filt2_out[i]
            if f2_val is not None and abs(f2_val) > 1e-12 and f1_val is not None:
                madh[i] = 100.0 * (f1_val - f2_val) / f2_val
            else:
                madh[i] = 0.0
...
def premier_stochastic_oscillator(
    highs: list[float],
    lows: list[float],
    closes: list[float],
    period: int = 8,
    ...
    Stoch = 100 * (Close - LLV(Low, Period)) / (HHV(High, Period) - LLV(Low, Period))
    N = 0.1 * (Stoch - 50)
    ...
    count = len(closes)
    out: list[float | None] = [None] * count
    if count == 0 or period <= 0 or smooth <= 0:
        return out

    raw_n: list[float | None] = [None] * count
    for i in range(count):
        if i >= period - 1:
            ...
            ll = min(l_win)
            rng = hh - ll
            stoch = 100.0 * (closes[i] - ll) / rng if rng > 1e-12 else 50.0
            raw_n[i] = 0.1 * (stoch - 50.0)

    # Double EMA smoothing of N
    s1 = ema_of_optional(raw_n, smooth)
    s2 = ema_of_optional(s1, smooth)

    for i in range(count):
        val = s2[i]
        if val is not None:
            clipped = max(-50.0, min(50.0, val))
            e_s = math.exp(clipped)
            out[i] = (e_s - 1.0) / (e_s + 1.0)

    return out


def apirine_tradj_ema(
    highs: list[float],
    lows: list[float],
    closes: list[float],
    periods: int = 20,
    ...
    """
    count = len(closes)
    tradj_ema_out: list[float | None] = [None] * count
    ema_ref_out: list[float | None] = [None] * count
    if count == 0 or periods <= 0 or pds <= 0:
        return tradj_ema_out, ema_ref_out

    tr_vals = true_range(highs, lows, closes)
    tr_clean = [0.0] * count
    for i in range(count):
        v = tr_vals[i]
        tr_clean[i] = v if v is not None else (highs[i] - lows[i])

    tradj = [0.0] * count
    for i in range(count):
        if i >= pds - 1:
            win = tr_clean[i - pds + 1 : i + 1]
            ll = min(win)
            hh = max(win)
            rng = hh - ll
            tradj[i] = (tr_clean[i] - ll) / rng if rng > 1e-12 else 0.0
        else:
            tradj[i] = 0.0

    mltp1 = 2.0 / (periods + 1.0)
    warmup = max(periods, pds)
    prev = closes[0]
    for i in range(count):
        rate = mltp1 * (1.0 + tradj[i] * mltp)
        prev = prev + rate * (closes[i] - prev)
        if i >= warmup - 1:
            tradj_ema_out[i] = prev

    ema_ref = ema(closes, periods)
    ema_ref_out = ema_ref
