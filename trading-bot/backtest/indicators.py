...
def macd(
    ...
    for i in range(n):
        if ef[i] is None or es[i] is None:
            continue
        line[i] = ef[i] - es[i]
    # Signal = EMA of macd line; ema() needs dense floats — build compact then map
    # Pine applies EMA on the series with Nones effectively starting after slow warmup.
...
def _ema_sparse(values: list[float | None], length: int) -> list[float | None]:
    """EMA over a series that may start with Nones; seed = SMA of first `length` values after warmup."""
    n = len(values)
    out: list[float | None] = [None] * n
    if length <= 0:
        return out
