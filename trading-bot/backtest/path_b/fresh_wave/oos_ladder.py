"""fresh-wave-v1 OOS stop-ladder on PASS_6m cells (ETH → SOL → BNB).

Strategy stop-ladder (not full 3×3):
1) Run ETH on all three PASS_6m cells
2) SOL only on cells that PASS ETH
3) BNB only on cells that PASS SOL

Frozen params/TF from fresh-wave-v1 BTC scoreboard. Gate: 6m Mode-A ≥ 1.2×
that symbol's B&H. Also report full(~2y). No paper, no param spray, no Claude.
"""
...
from backtest.path_b.fresh_wave import RESEARCH_ID
...
# BTC PASS_6m cells — priority order (frozen TF/params)
PASS_6M_CELLS: tuple[tuple[str, str], ...] = (
    ("ichimoku-cloud-trend-v1", "2h"),
    ("obv-ema-trend-v1", "1d"),
    ("nr7-breakout-v1", "2d"),
)
...
def _run_one(
    ...
    try:
        ...
        # patch print label with real tf
...
def run_oos_ladder(*, years: float = 2.5, refresh: bool = False) -> list[LadderCell]:
    ...
    # Track which cells passed at each rung for stop-ladder
    ...
    for rung, symbol in enumerate(SYMBOL_LADDER):
        if rung == 0:
            ...
            # Also record SKIPs for non-candidates so report is complete
        ...
        if not rung_pass and rung < len(SYMBOL_LADDER) - 1:
            # Still emit SKIPs for remaining symbols for completeness
...
def write_oos_ladder_report(
    ...
    lines: list[str] = [
        ...
        f"_Generated: {now}_",
        "",
        "**RESEARCH ONLY — OOS stop-ladder on fresh-wave-v1 PASS_6m cells. "
    ...
    for sym in SYMBOL_LADDER:
        ...
        for i, (sid, tf) in enumerate(PASS_6M_CELLS, 1):
            ...
            if r is None or r.skipped or r.error or not r.metrics:
                status = "SKIP" if r and r.skipped else ("ERROR" if r and r.error else "—")
                lines.append(
                    f"| {i} | {sid} | {tf} | — | — | — | — | — | {status} |"
                )
                continue
