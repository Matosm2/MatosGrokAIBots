"""CLI: python -m backtest.path_b.fresh_wave_v10 [--refresh] [--tfs ...] [--prefer-coarse]"""

from __future__ import annotations

import argparse

from backtest.path_b.fresh_wave_v10 import COARSE_FIRST_TFS, RESEARCH_ID, STRATEGY_IDS
from backtest.path_b.fresh_wave_v10.sweep import run_fresh_wave_v10, write_scoreboard
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Comma TFs for oscillator sprays (default: coarse-first 16)",
    )
    ap.add_argument(
        "--prefer-coarse",
        action="store_true",
        help="Restrict oscillator sprays to 4h→15m band; session TFs unchanged",
    )
    args = ap.parse_args()
    ...
    print(
        f"{RESEARCH_ID}: VWAP/SMI/Woodie/Chaikin/Laguerre "
        f"(ids={list(STRATEGY_IDS)}; tfs={len(tfs)})",
        flush=True,
    )
    results = run_fresh_wave_v10(
        years=args.years,
