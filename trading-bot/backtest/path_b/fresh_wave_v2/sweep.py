"""fresh-wave-v2 harness: 5 strategies × 16 TFs = 80 cells (BTCUSDT).
...
from backtest.path_b.fresh_wave_v2.vortex_trend_v1 import compute_signals as vortex_signals
...
def _signals_for(
    ...
    if strategy_id == "psar-trend-v1":
        return psar_signals(bars)
    if strategy_id == "cci-mr-v1":
        return cci_signals(bars)
    if strategy_id == "aroon-trend-v1":
        return aroon_signals(bars)
    if strategy_id == "williams-r-mr-v1":
        return williams_signals(bars)
    if strategy_id == "vortex-trend-v1":
        return vortex_signals(bars)
    ...
    *,
    ...
    for tf in tfs:
        for sid in STRATEGY_IDS:
            print(f"[fresh-wave-v2] {sid} @ {tf} ...", flush=True)
...
def write_scoreboard(results: list[CellResult], path: Path | None = None) -> Path:
    ...
    path = path or (RESULTS_DIR / "fresh-wave-v2-scoreboard.md")
