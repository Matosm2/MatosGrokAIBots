"""fresh-wave-v3 OOS stop-ladder on PASS_6m cells (ETH → SOL → BNB).
...
Frozen params/TF from fresh-wave-v3 BTC scoreboard. Gate: 6m Mode-A ≥ 1.2×
...
from backtest.path_b.fresh_wave_v3 import RESEARCH_ID
from backtest.path_b.fresh_wave_v3.donchian_breakout_v1 import (
    compute_signals as donchian_signals,
)
from backtest.path_b.fresh_wave_v3.schaff_stc_v1 import compute_signals as schaff_signals
...
# BTC PASS_6m cells — sample-size priority (more bars first); adx/elder/tsi excluded
PASS_6M_CELLS: tuple[tuple[str, str], ...] = (
    ("schaff-stc-v1", "12h"),
    ("schaff-stc-v1", "1d"),
    ("donchian-breakout-v1", "1d"),
    ("schaff-stc-v1", "2d"),
)
...
def _signals_for(
    ...
    if strategy_id == "schaff-stc-v1":
        buys, sells = schaff_signals(bars)
        return buys, sells, None
    if strategy_id == "donchian-breakout-v1":
        buys, sells = donchian_signals(bars)
        return buys, sells, None
...
def write_oos_ladder_report(
    ...
    lines: list[str] = [
        ...
        f"_Generated: {now}_",
        "",
        "**RESEARCH ONLY — OOS stop-ladder on fresh-wave-v3 PASS_6m cells. "
