"""fresh-wave-v5 OOS stop-ladder on PASS_6m cells (ETH → SOL → BNB).

Strategy stop-ladder (not full 3×3):
1) Run ETH on all PASS_6m cells (sample-size priority order)
2) SOL only on cells that PASS ETH
3) BNB only on cells that PASS SOL

Frozen params/TF from fresh-wave-v5 BTC scoreboard. Gate: 6m Mode-A ≥ 1.2×
...
from backtest.path_b.fresh_wave_v5 import RESEARCH_ID
...
# BTC PASS_6m cells — sample-size priority (more 6m trades first).
# Labels match scoreboard cell keys (tf|params). Elder/LinReg/UO excluded.
PASS_6M_CELLS: tuple[tuple[str, str], ...] = (
    ("cmf-flow-v1", "4h|p20|A"),  # n=46
    ("cmf-flow-v1", "7h|p20|A"),  # n=34
    ("cmf-flow-v1", "7h|p21|A"),  # n=27
    ("mfi-only-v1", "2d|14|15/85"),  # n=1
    ("mfi-only-v1", "2d|14|20/80"),  # n=1
)
...
class LadderCell:
    symbol: str
    strategy_id: str
    tf: str  # full cell key (e.g. 4h|p20|A)
    gate_6m: str = "FAIL"
    gate_full: str = "FAIL"
    skipped: bool = False
    skip_reason: str = ""
...
def _signals_for(
    ...
    if strategy_id == "cmf-flow-v1":
        # {tf}|p{period}|{mode}
        period = int(parts[1].lstrip("p"))
        mode = parts[2]
        buys, sells = cmf_signals(
            bars, CmfFlowParams(period=period, mode=mode, channel_n=20)
        )
        return buys, sells, None
    if strategy_id == "mfi-only-v1":
        # {tf}|{length}|{lo}/{hi}
        length = int(parts[1])
        lo_s, hi_s = parts[2].split("/")
        buys, sells = mfi_signals(
            bars,
            MfiOnlyParams(
                length=length, low_level=float(lo_s), high_level=float(hi_s)
            ),
        )
        return buys, sells, None
...
def write_oos_ladder_report(
    ...
    lines: list[str] = [
        ...
        f"_Generated: {now}_",
        "",
        "**RESEARCH ONLY — OOS stop-ladder on fresh-wave-v5 PASS_6m cells. "
        ...
        "- Hard-stop (not OOS): elder-triple-screen-fi-v1, linreg-r2-v1, "
    ...
    if survivors:
        for r in survivors:
            ...
            lines.append(
                ...
                f"(BNB ratio={g.ratio:.3f} n={g.trades})"
            )
    else:
        eth_pass = [
            r
