"""CLI: python -m backtest.path_b.fresh_wave_v6 [--refresh] [--tfs ...] [--prefer-coarse] [--oos-ladder]"""

from __future__ import annotations

import argparse

from backtest.path_b.fresh_wave_v6 import COARSE_FIRST_TFS, RESEARCH_ID, STRATEGY_IDS
from backtest.path_b.fresh_wave_v6.sweep import run_fresh_wave_v6, write_scoreboard
...
def main() -> None:
    ...
    ap.add_argument(
        "--oos-ladder",
        action="store_true",
        help="Stop-ladder OOS: ETH→SOL→BNB on PASS_6m cell (twiggs 3h|p21|B)",
    )
    args = ap.parse_args()

    if args.oos_ladder:
        from backtest.path_b.fresh_wave_v6.oos_ladder import (
            OOS_RESEARCH_ID,
            run_oos_ladder,
    ...
    print(
        f"{RESEARCH_ID}: Mass/KST/Twiggs/DeM/Darvas "
        f"(ids={list(STRATEGY_IDS)}; tfs={len(tfs)})",
        flush=True,
    )
    results = run_fresh_wave_v6(
        years=args.years,
