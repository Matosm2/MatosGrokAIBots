"""CLI: python -m backtest.path_b.fresh_wave_v7 [--refresh] [--tfs ...] [--prefer-coarse] [--oos-ladder]"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--oos-ladder",
        action="store_true",
        help="Stop-ladder OOS: ETH→SOL→BNB on PASS_6m elder-impulse cells",
    )
    args = ap.parse_args()

    if args.oos_ladder:
        from backtest.path_b.fresh_wave_v7.oos_ladder import (
            OOS_RESEARCH_ID,
            run_oos_ladder,
