"""fresh-wave-v7 OOS stop-ladder on PASS_6m cells (ETH → SOL → BNB).

Frozen cells (BTC PASS_6m only):
1. elder-impulse-v1 @ 1d|ema13|cancel2 — BTC 1.231× n=14
2. elder-impulse-v1 @ 2d|ema13|cancel2 — BTC 1.826× n=6
...
from backtest.path_b.fresh_wave_v7 import ELDER_CANCEL, ELDER_EMA, RESEARCH_ID
...
# BTC PASS_6m cells — frozen (sample-size priority: more 6m trades first).
PASS_6M_CELLS: tuple[tuple[str, str], ...] = (
    ("elder-impulse-v1", "1d|ema13|cancel2"),  # BTC 1.231× n=14
    ("elder-impulse-v1", "2d|ema13|cancel2"),  # BTC 1.826× n=6
)
...
class LadderCell:
    symbol: str
    strategy_id: str
    tf: str  # full cell key (e.g. 1d|ema13|cancel2)
    gate_6m: str = "FAIL"
    gate_full: str = "FAIL"
    skipped: bool = False
    skip_reason: str = ""
...
def _signals_for(
    ...
    if strategy_id == "elder-impulse-v1":
        # {tf}|ema{n}|cancel{k}
        ema_n = int(parts[1].lstrip("ema"))
        cancel_n = int(parts[2].lstrip("cancel"))
        buys, sells = elder_signals(
            bars, ElderImpulseParams(ema_length=ema_n, cancel_bars=cancel_n)
        )
...
def write_oos_ladder_report(
    ...
    path = path or (RESULTS_DIR / "fresh-wave-v7-oos-ladder.md")
    ...
    lines: list[str] = [
        ...
        f"_Generated: {now}_",
        "",
        "**RESEARCH ONLY — OOS stop-ladder on fresh-wave-v7 PASS_6m cells. "
        ...
        f"- Params frozen: Elder Impulse EMA({ELDER_EMA}) cancel({ELDER_CANCEL}) "
        "+ MACD-hist; no FI; no market-on-green",
        "- Hard-stop (not OOS): "
        + ", ".join(HARD_STOP_IDS),
        "",
        "## Cells (frozen)",
    ...
    cell_notes = {
        ("elder-impulse-v1", "1d|ema13|cancel2"): "BTC PASS_6m 1.231× n=14",
        ("elder-impulse-v1", "2d|ema13|cancel2"): "BTC PASS_6m 1.826× n=6",
    }
