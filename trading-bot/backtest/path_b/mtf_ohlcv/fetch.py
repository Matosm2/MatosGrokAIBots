...
def materialize_symbol(
    ...
    *,
    ...
    """Cache native 1h, and aggregate 4h from 1h or cache directly."""
    tfs = tfs or ("1h", "4h")
    want = [normalize_tf(t) for t in tfs]
    out: dict[str, list[Bar]] = {}

    # If only 1h and/or 4h requested, fetch native 1h and native 4h directly (fast & robust)
    for tf in want:
        if tf in ("1h", "4h"):
            print(f"[mtf_ohlcv] loading native {tf} for {symbol} (~{years:g}y)...", flush=True)
            bars = load_or_fetch(symbol, tf, years=years, refresh=refresh)
            out[tf] = bars
            MTF_CACHE_DIR.mkdir(parents=True, exist_ok=True)
            path = _cache_path(MTF_CACHE_DIR, symbol.upper(), tf)
            bars_to_csv(path, bars)
            ...
            print(f"[mtf_ohlcv] {tf} -> {len(bars)} bars", flush=True)

    # Check if other TFs were requested (e.g. 5m, 1d, 15m)
    remaining = [t for t in want if t not in out]
    if not remaining:
        return out

    # Fallback to standard 5m+1d aggregation for remaining TFs
