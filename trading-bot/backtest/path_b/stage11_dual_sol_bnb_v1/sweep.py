"""stage11-dual-sol-bnb-v1 harness: Sweep and score five strategies (BTC -> ETH -> SOL -> BNB).

LOCKED ENCODE ORDER (exactly 4 + optional 5th VQI):
1. chande-trendscore-zero-cross-v1
2. pee-tdi-direction-zero-v1
3. ehlers-leading-netlead-ema-v1
4. gmma-osc-zero-cross-v1
5. vqi-sum-sma-cross-v1
...
from backtest.path_b.stage11_dual_sol_bnb_v1 import (
    DEFAULT_SYMBOLS,
    RESEARCH_ID,
    STRATEGY_IDS,
)
...
# ---------------------------------------------------------------------------
# Strategy 1: chande-trendscore-zero-cross-v1
# ---------------------------------------------------------------------------
...
def run_chande_cells(
    ...
    sid = "chande-trendscore-zero-cross-v1"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, ChandeTrendScoreParams]] = [
        ("mode_a|(11,10)", ChandeTrendScoreParams(mode="mode_a", start_lag=11, width=10)),
        ("mode_a|(8,8)", ChandeTrendScoreParams(mode="mode_a", start_lag=8, width=8)),
        ("mode_a|(15,7)", ChandeTrendScoreParams(mode="mode_a", start_lag=15, width=7)),
        ("mode_b|(11,10,ema5)", ChandeTrendScoreParams(mode="mode_b", start_lag=11, width=10, ema_len=5)),
    ]
...
# ---------------------------------------------------------------------------
# Strategy 2: pee-tdi-direction-zero-v1
# ---------------------------------------------------------------------------
...
def run_pee_cells(
    ...
    sid = "pee-tdi-direction-zero-v1"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, PeeTdiDirectionParams]] = [
        ("mode_a|(N20)", PeeTdiDirectionParams(mode="mode_a", n_len=20)),
        ("mode_a|(N14)", PeeTdiDirectionParams(mode="mode_a", n_len=14)),
        ("mode_a|(N25)", PeeTdiDirectionParams(mode="mode_a", n_len=25)),
        ("mode_b|(N20)", PeeTdiDirectionParams(mode="mode_b", n_len=20)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            sol_ok, sol_msg = validate_pee_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_pee_bnb_smoke(p, tf)
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = pee_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                eth_cell = eth_results.get(cell_key) if eth_results else None
...
# ---------------------------------------------------------------------------
# Strategy 3: ehlers-leading-netlead-ema-v1
# ---------------------------------------------------------------------------


def run_ehlers_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    eth_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "ehlers-leading-netlead-ema-v1"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, EhlersLeadingParams]] = [
        ("mode_a|(a1_0.25,a2_0.50)", EhlersLeadingParams(mode="mode_a", a1=0.25, a2=0.50)),
        ("mode_a|(a1_0.20,a2_0.50)", EhlersLeadingParams(mode="mode_a", a1=0.20, a2=0.50)),
        ("mode_a|(a1_0.25,a2_0.33)", EhlersLeadingParams(mode="mode_a", a1=0.25, a2=0.33)),
        ("mode_a|(a1_0.33,a2_0.50)", EhlersLeadingParams(mode="mode_a", a1=0.33, a2=0.50)),
        ("mode_b|(a1_0.25,a2_0.50)", EhlersLeadingParams(mode="mode_b", a1=0.25, a2=0.50)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            sol_ok, sol_msg = validate_ehlers_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_ehlers_bnb_smoke(p, tf)
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = ehlers_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                eth_cell = eth_results.get(cell_key) if eth_results else None
...
# ---------------------------------------------------------------------------
# Strategy 4: gmma-osc-zero-cross-v1
# ---------------------------------------------------------------------------
...
def run_gmma_cells(
    ...
    sid = "gmma-osc-zero-cross-v1"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, GmmaOscParams]] = [
        ("mode_a|(classic12)", GmmaOscParams(mode="mode_a")),
        ("mode_b|(classic12,sig15)", GmmaOscParams(mode="mode_b", sig_len=15)),
        ("mode_b|(classic12,sig10)", GmmaOscParams(mode="mode_b", sig_len=10)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            sol_ok, sol_msg = validate_gmma_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_gmma_bnb_smoke(p, tf)
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = gmma_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                eth_cell = eth_results.get(cell_key) if eth_results else None
...
# ---------------------------------------------------------------------------
# Strategy 5: vqi-sum-sma-cross-v1
# ---------------------------------------------------------------------------
...
def run_vqi_cells(
    ...
    sid = "vqi-sum-sma-cross-v1"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, VqiSumSmaParams]] = [
        ("mode_a|(fast9)", VqiSumSmaParams(mode="mode_a", sma_fast=9)),
        ("mode_a|(fast5)", VqiSumSmaParams(mode="mode_a", sma_fast=5)),
        ("mode_a|(fast14)", VqiSumSmaParams(mode="mode_a", sma_fast=14)),
        ("mode_b|(fast9,slow200)", VqiSumSmaParams(mode="mode_b", sma_fast=9, sma_slow=200)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            sol_ok, sol_msg = validate_vqi_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_vqi_bnb_smoke(p, tf)
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = vqi_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                eth_cell = eth_results.get(cell_key) if eth_results else None
...
def run_strategy_cells(
    ...
    if strategy_id == "chande-trendscore-zero-cross-v1":
        return run_chande_cells(symbol, bars_by_tf, active_keys, eth_results)
    elif strategy_id == "pee-tdi-direction-zero-v1":
        return run_pee_cells(symbol, bars_by_tf, active_keys, eth_results)
    elif strategy_id == "ehlers-leading-netlead-ema-v1":
        return run_ehlers_cells(symbol, bars_by_tf, active_keys, eth_results)
    elif strategy_id == "gmma-osc-zero-cross-v1":
        return run_gmma_cells(symbol, bars_by_tf, active_keys, eth_results)
    elif strategy_id == "vqi-sum-sma-cross-v1":
        return run_vqi_cells(symbol, bars_by_tf, active_keys, eth_results)
    else:
        raise ValueError(f"Unknown strategy_id: {strategy_id}")


def run_stage11_dual_sol_bnb_v1(
    ...
    for symbol in symbols:
        print(f"\n{'='*70}\n[stage11-dual-sol-bnb-v1] Evaluating Symbol: {symbol}\n{'='*70}", flush=True)
        ...
        for sid in STRATEGY_IDS:
            param_grid_keys = {
                "chande-trendscore-zero-cross-v1": ["mode_a|(11,10)", "mode_a|(8,8)", "mode_a|(15,7)", "mode_b|(11,10,ema5)"],
                "pee-tdi-direction-zero-v1": ["mode_a|(N20)", "mode_a|(N14)", "mode_a|(N25)", "mode_b|(N20)"],
                "ehlers-leading-netlead-ema-v1": ["mode_a|(a1_0.25,a2_0.50)", "mode_a|(a1_0.20,a2_0.50)", "mode_a|(a1_0.25,a2_0.33)", "mode_a|(a1_0.33,a2_0.50)", "mode_b|(a1_0.25,a2_0.50)"],
                "gmma-osc-zero-cross-v1": ["mode_a|(classic12)", "mode_b|(classic12,sig15)", "mode_b|(classic12,sig10)"],
                "vqi-sum-sma-cross-v1": ["mode_a|(fast9)", "mode_a|(fast5)", "mode_a|(fast14)", "mode_b|(fast9,slow200)"],
            }[sid]
...
def write_scoreboard(
    ...
    md_path = output_dir / "stage11-dual-sol-bnb-v1-scoreboard.md"
    csv_path = output_dir / "stage11-dual-sol-bnb-v1-scoreboard.csv"
