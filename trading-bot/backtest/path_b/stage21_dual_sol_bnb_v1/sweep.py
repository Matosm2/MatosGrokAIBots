"""stage21-dual-sol-bnb-v1 harness: Sweep and score strategies (BTC -> ETH -> SOL -> BNB).

LOCKED ENCODE ORDER (4 strategies after Parkinson #4 dropped for BTC-smoke redundancy):
1. chaikin-volatility-dir
2. outside-bar-polarity
3. ulcer-index-recover-dir
4. inside-bar-breakout

Design Bias:
  BTC LEAD PRIMARY without over-damp (stage12/15/18/20 rhyme).
...
from backtest.path_b.stage21_dual_sol_bnb_v1 import (
    DEFAULT_SYMBOLS,
    RESEARCH_ID,
    STRATEGY_IDS,
)
...
# ---------------------------------------------------------------------------
# Strategy 1: chaikin-volatility-dir
# ---------------------------------------------------------------------------

def run_chaikin_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "chaikin-volatility-dir"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, ChaikinParams]] = [
        ("mode_a|(ema10,roc10,dir1)", ChaikinParams(mode="mode_a", ema_len=10, roc_len=10, dir_len=1)),
        ("mode_a|(ema14,roc10,dir1)", ChaikinParams(mode="mode_a", ema_len=14, roc_len=10, dir_len=1)),
        ("mode_a|(ema10,roc5,dir1)", ChaikinParams(mode="mode_a", ema_len=10, roc_len=5, dir_len=1)),
        ("mode_b|(ema10,roc10,dir1,cv0)", ChaikinParams(mode="mode_b", ema_len=10, roc_len=10, dir_len=1)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_chaikin_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_chaikin_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_chaikin_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_chaikin_bnb_smoke(p, tf)
            ...
            try:
                buys, sells, stops = chaikin_signals(bars, p)
...
# ---------------------------------------------------------------------------
# Strategy 2: outside-bar-polarity
# ---------------------------------------------------------------------------

def run_outside_bar_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "outside-bar-polarity"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, OutsideBarParams]] = [
        ("mode_a|(mid0.5,conf1)", OutsideBarParams(mode="mode_a", mid_frac=0.5, confirm_bars=1)),
        ("mode_a|(mid0.6,conf1)", OutsideBarParams(mode="mode_a", mid_frac=0.6, confirm_bars=1)),
        ("mode_a|(mid0.5,conf2)", OutsideBarParams(mode="mode_a", mid_frac=0.5, confirm_bars=2)),
        ("mode_b|(mid0.5,conf1,follth)", OutsideBarParams(mode="mode_b", mid_frac=0.5, confirm_bars=1, require_follow_through=True)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_outside_bar_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_outside_bar_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_outside_bar_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_outside_bar_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = outside_bar_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                prior_cell = prior_results.get(cell_key) if prior_results else None
...
# ---------------------------------------------------------------------------
# Strategy 3: ulcer-index-recover-dir
# ---------------------------------------------------------------------------
...
def run_ulcer_cells(
    ...
    sid = "ulcer-index-recover-dir"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, UlcerParams]] = [
        ("mode_a|(ui14,dir1)", UlcerParams(mode="mode_a", ui_len=14, dir_len=1)),
        ("mode_a|(ui10,dir1)", UlcerParams(mode="mode_a", ui_len=10, dir_len=1)),
        ("mode_a|(ui20,dir1)", UlcerParams(mode="mode_a", ui_len=20, dir_len=1)),
        ("mode_b|(ui14,dir1,sma5)", UlcerParams(mode="mode_b", ui_len=14, dir_len=1, sma_len=5)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_ulcer_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_ulcer_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_ulcer_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_ulcer_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = ulcer_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                prior_cell = prior_results.get(cell_key) if prior_results else None
...
def run_inside_bar_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "inside-bar-breakout"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, InsideBarParams]] = [
        ("mode_a|(cancel5,k0.0)", InsideBarParams(mode="mode_a", cancel_bars=5, k=0.0)),
        ("mode_a|(cancel3,k0.0)", InsideBarParams(mode="mode_a", cancel_bars=3, k=0.0)),
        ("mode_a|(cancel8,k0.0)", InsideBarParams(mode="mode_a", cancel_bars=8, k=0.0)),
        ("mode_b|(cancel5,k0.5)", InsideBarParams(mode="mode_b", cancel_bars=5, k=0.5)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_inside_bar_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_inside_bar_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_inside_bar_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_inside_bar_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = inside_bar_signals(bars, p)
                cell.metrics, cell.tiny_n_kill, cell.thin_n_flag = _eval_windows(
                    symbol, sid, bars, buys, sells, stops
                )
                prior_cell = prior_results.get(cell_key) if prior_results else None
...
def run_stage21_dual_sol_bnb_v1(
    ...
    for s_idx, symbol in enumerate(symbols):
        print(f"\n{'='*70}\n[stage21-dual-sol-bnb-v1] Evaluating Symbol: {symbol} (LADDER STEP {s_idx + 1}/4)\n{'='*70}", flush=True)
        ...
        for sid in STRATEGY_IDS:
            param_grid_keys = {
                "chaikin-volatility-dir": ["mode_a|(ema10,roc10,dir1)", "mode_a|(ema14,roc10,dir1)", "mode_a|(ema10,roc5,dir1)", "mode_b|(ema10,roc10,dir1,cv0)"],
                "outside-bar-polarity": ["mode_a|(mid0.5,conf1)", "mode_a|(mid0.6,conf1)", "mode_a|(mid0.5,conf2)", "mode_b|(mid0.5,conf1,follth)"],
                "ulcer-index-recover-dir": ["mode_a|(ui14,dir1)", "mode_a|(ui10,dir1)", "mode_a|(ui20,dir1)", "mode_b|(ui14,dir1,sma5)"],
                "inside-bar-breakout": ["mode_a|(cancel5,k0.0)", "mode_a|(cancel3,k0.0)", "mode_a|(cancel8,k0.0)", "mode_b|(cancel5,k0.5)"],
            }[sid]
            ...
            if active_keys is None or any(k.startswith(f"{sid}@") for k in active_keys):
                bars = _get_bars()
                if sid == "chaikin-volatility-dir":
                    res = run_chaikin_cells(symbol, bars, active_keys=active_keys, prior_results=prior_results_by_key)
                    sym_results.extend(res)
                    all_results.extend(res)
                elif sid == "outside-bar-polarity":
                    res = run_outside_bar_cells(symbol, bars, active_keys=active_keys, prior_results=prior_results_by_key)
                    sym_results.extend(res)
                    all_results.extend(res)
                elif sid == "ulcer-index-recover-dir":
                    res = run_ulcer_cells(symbol, bars, active_keys=active_keys, prior_results=prior_results_by_key)
                    sym_results.extend(res)
                    all_results.extend(res)
                elif sid == "inside-bar-breakout":
                    res = run_inside_bar_cells(symbol, bars, active_keys=active_keys, prior_results=prior_results_by_key)
                    sym_results.extend(res)
                    all_results.extend(res)
...
def write_scoreboard(results: list[CellResult]) -> tuple[Path, Path]:
    ...
    md_path = RESULTS_DIR / "stage21-dual-sol-bnb-v1-scoreboard.md"
    csv_path = RESULTS_DIR / "stage21-dual-sol-bnb-v1-scoreboard.csv"
    ...
    for r in results:
        ...
        if r.skipped:
            ...
            md_lines.append(
                f"| {r.symbol} | `{r.strategy_id}` | {r.tf} | `{r.mode_params}` | **{r.gate_6m}** | {g6.return_pct:.2f}% | {g6.bh_return_pct:.2f}% | {g6.ratio:.3f}x | {g6.trades} | {g6.win_rate_pct:.1f}% | {r.gate_full} | {gf.return_pct:.2f}% | {gf.ratio:.3f}x | {o6.return_pct:.2f}% | {of.return_pct:.2f}% | {r.btc_smoke} | {r.eth_smoke} | {r.sol_smoke} | {r.bnb_smoke} | {'; '.join(r.notes) if r.notes else '—'} |"
            )
