"""stage26-dual-sol-bnb-v1 harness: Sweep and score strategies (BTC -> ETH -> SOL -> BNB).
...
Design Bias:
  ETH-after-BTC PRIMARY CRITICAL (stage25 NHNL BTC 1.687x -> ETH 1.085x FAIL; n=8 THIN).
  Need BTC clear >= 1.20 without over-damp PLUS ETH >= 1.2x with denser n >> 9.
  Keep SOL-after + BNB-survival.
...
from backtest.path_b.stage26_dual_sol_bnb_v1 import (
    DEFAULT_SYMBOLS,
    RESEARCH_ID,
    STRATEGY_IDS,
)
...
def _finish(cell: CellResult, prior_cell: CellResult | None = None) -> CellResult:
    ...
    if cell.symbol == "ETHUSDT":
        cell.notes.append("ETH hard filter (ETH-after-BTC PRIMARY CRITICAL)")
...
# ---------------------------------------------------------------------------
# Strategy 1: katsanos-fve-zero-cross
# ---------------------------------------------------------------------------
...
def run_fve_cells(
    ...
    sid = "katsanos-fve-zero-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, KatsanosFveParams]] = [
        ("mode_a|(S22)", KatsanosFveParams(mode="mode_a", samples=22)),
        ("mode_a|(S14)", KatsanosFveParams(mode="mode_a", samples=14)),
        ("mode_a|(S18)", KatsanosFveParams(mode="mode_a", samples=18)),
        ("mode_a|(S30)", KatsanosFveParams(mode="mode_a", samples=30)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_fve_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_fve_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_fve_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_fve_bnb_smoke(p, tf)
            ...
            try:
                buys, sells, stops = fve_signals(bars, p)
...
# ---------------------------------------------------------------------------
# Strategy 2: ehlers-convolution-zero
# ---------------------------------------------------------------------------

def run_convolution_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "ehlers-convolution-zero"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, EhlersConvolutionParams]] = [
        ("mode_a|(L18,T0.05)", EhlersConvolutionParams(mode="mode_a", lookback=18, thr=0.05)),
        ("mode_a|(L13,T0.05)", EhlersConvolutionParams(mode="mode_a", lookback=13, thr=0.05)),
        ("mode_a|(L26,T0.05)", EhlersConvolutionParams(mode="mode_a", lookback=26, thr=0.05)),
        ("mode_a|(L39,T0.05)", EhlersConvolutionParams(mode="mode_a", lookback=39, thr=0.05)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_conv_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_conv_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_conv_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_conv_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = conv_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Strategy 3: hilbert-inst-trendline-cross
# ---------------------------------------------------------------------------

def run_trendline_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "hilbert-inst-trendline-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, HilbertTrendlineParams]] = [
        ("mode_a|(close)", HilbertTrendlineParams(mode="mode_a", source="close")),
        ("mode_a|(hl2)", HilbertTrendlineParams(mode="mode_a", source="hl2")),
        ("mode_b|(close)", HilbertTrendlineParams(mode="mode_b", source="close")),
        ("mode_b|(hl2)", HilbertTrendlineParams(mode="mode_b", source="hl2")),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_ht_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_ht_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_ht_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_ht_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = ht_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Strategy 4: elder-safezone-trail-flip
# ---------------------------------------------------------------------------

def run_safezone_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "elder-safezone-trail-flip"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, ElderSafeZoneParams]] = [
        ("mode_a|(ema22,N10,k2.5)", ElderSafeZoneParams(mode="mode_a", ema_len=22, n=10, k=2.5)),
        ("mode_a|(ema14,N10,k2.5)", ElderSafeZoneParams(mode="mode_a", ema_len=14, n=10, k=2.5)),
        ("mode_a|(ema22,N8,k2.0)", ElderSafeZoneParams(mode="mode_a", ema_len=22, n=8, k=2.0)),
        ("mode_a|(ema30,N15,k3.0)", ElderSafeZoneParams(mode="mode_a", ema_len=30, n=15, k=3.0)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_safezone_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_safezone_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_safezone_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_safezone_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = safezone_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
def run_stage26_dual_sol_bnb_v1(
    ...
    for stage_idx, symbol in enumerate(symbols):
        print(f"\n{'-'*70}\n[stage26-dual-sol-bnb-v1] Ladder Stage {stage_idx+1}/4: {symbol}\n{'-'*70}", flush=True)
        ...
        if active_keys is not None and len(active_keys) == 0:
            ...
            if symbol not in bars_by_symbol_and_tf:
                print(f"[stage26-dual-sol-bnb-v1] Materializing OHLCV data for {symbol} ({years}y)...", flush=True)
        ...
        s1_cells = run_fve_cells(symbol, bars_by_tf, active_keys, prior_results_by_key)
        s2_cells = run_convolution_cells(symbol, bars_by_tf, active_keys, prior_results_by_key)
        s3_cells = run_trendline_cells(symbol, bars_by_tf, active_keys, prior_results_by_key)
        s4_cells = run_safezone_cells(symbol, bars_by_tf, active_keys, prior_results_by_key)
        ...
        print(
            f"\n[stage26-dual-sol-bnb-v1] {symbol} Stage Summary: "
        ...
        if not active_keys and stage_idx < len(symbols) - 1:
            print(
                f"[stage26-dual-sol-bnb-v1] No strategy passed {symbol} 6m gate. "
...
def write_scoreboard(results: list[CellResult]) -> tuple[Path, Path]:
    ...
    for cand, syms in passed_symbols_by_candidate.items():
        if set(syms) == set(DEFAULT_SYMBOLS):
