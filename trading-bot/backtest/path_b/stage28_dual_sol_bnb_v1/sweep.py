"""stage28-dual-sol-bnb-v1 harness: Sweep and score strategies (BTC -> ETH -> SOL -> BNB).
...
Design Bias:
  BTC LEAD PRIMARY CRITICAL — push past FVE 1.055x / VR 0.698x to >= 1.20 without over-damp.
...
from backtest.path_b.stage28_dual_sol_bnb_v1 import (
    DEFAULT_SYMBOLS,
    RESEARCH_ID,
    STRATEGY_IDS,
)
...
# ---------------------------------------------------------------------------
# Strategy 1: ehlers-ec-ema-cross
# ---------------------------------------------------------------------------

def run_ehlers_ec_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "ehlers-ec-ema-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, EhlersEcParams]] = [
        ("mode_a|(len20,gain50,thr0)", EhlersEcParams(mode="mode_a", length=20, gain_limit=50, thresh=0.0)),
        ("mode_a|(len12,gain22,thr0)", EhlersEcParams(mode="mode_a", length=12, gain_limit=22, thresh=0.0)),
        ("mode_a|(len32,gain50,thr0)", EhlersEcParams(mode="mode_a", length=32, gain_limit=50, thresh=0.0)),
        ("mode_b|(len20,gain50,thr0.75)", EhlersEcParams(mode="mode_b", length=20, gain_limit=50, thresh=0.75)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_ec_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_ec_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_ec_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_ec_bnb_smoke(p, tf)
            ...
            try:
                buys, sells, stops = ec_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
                prior_c = prior_results.get(cell_key) if prior_results else None
                cell = _finish(cell, prior_c)
...
# ---------------------------------------------------------------------------
# Strategy 2: vervoort-zlha-typ-cross
# ---------------------------------------------------------------------------

def run_vervoort_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "vervoort-zlha-typ-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, VervoortZlhaParams]] = [
        ("mode_a|(N34)", VervoortZlhaParams(mode="mode_a", n=34)),
        ("mode_a|(N21)", VervoortZlhaParams(mode="mode_a", n=21)),
        ("mode_a|(N55)", VervoortZlhaParams(mode="mode_a", n=55)),
        ("mode_b|(N34_rising)", VervoortZlhaParams(mode="mode_b", n=34)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_vervoort_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_vervoort_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_vervoort_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_vervoort_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = vervoort_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Strategy 3: ehlers-fir-zl-price-cross
# ---------------------------------------------------------------------------
...
def run_fir_cells(
    ...
    sid = "ehlers-fir-zl-price-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, EhlersFirZlParams]] = [
        ("mode_a|(src_close_zl9.5)", EhlersFirZlParams(mode="mode_a", src="close", denom="9.5")),
        ("mode_a|(src_hl2_zl9.5)", EhlersFirZlParams(mode="mode_a", src="hl2", denom="9.5")),
        ("mode_b|(src_close_fir12)", EhlersFirZlParams(mode="mode_b", src="close", denom="12")),
        ("mode_b|(src_hl2_fir12)", EhlersFirZlParams(mode="mode_b", src="hl2", denom="12")),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_fir_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_fir_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_fir_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_fir_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = fir_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
def run_dv2_cells(
    ...
    sid = "dv2-varadi-midline"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, Dv2VaradiParams]] = [
        ("mode_a|(rank100_mid50)", Dv2VaradiParams(mode="mode_a", rank_len=100, mid=50.0)),
        ("mode_a|(rank63_mid50)", Dv2VaradiParams(mode="mode_a", rank_len=63, mid=50.0)),
        ("mode_a|(rank126_mid50)", Dv2VaradiParams(mode="mode_a", rank_len=126, mid=50.0)),
        ("mode_b|(rank100_reclaim40)", Dv2VaradiParams(mode="mode_b", rank_len=100, mid=50.0)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_dv2_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_dv2_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_dv2_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_dv2_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = dv2_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Master Ladder Runner
# ---------------------------------------------------------------------------

def run_stage28_dual_sol_bnb_v1(
    symbols: tuple[str, ...] = DEFAULT_SYMBOLS,
    years: float = 2.5,
    refresh: bool = False,
) -> list[CellResult]:
    print(
        f"[stage28-dual-sol-bnb-v1] Initializing Path B ladder sweep across {symbols}...\n"
        f"LOCKED ENCODE ORDER (4):\n"
        f"  1. ehlers-ec-ema-cross\n"
        f"  2. vervoort-zlha-typ-cross\n"
        f"  3. ehlers-fir-zl-price-cross\n"
        f"  4. dv2-varadi-midline\n"
        f"Stop-ladder rule: BTC PASS_6m >= 1.2x B&H (and n > 5) -> ETH -> SOL -> BNB.\n"
        f"Design bias: BTC LEAD PRIMARY CRITICAL — clear past FVE 1.055x / VR 0.698x to >= 1.20.\n",
        flush=True,
    )
    ...
    for stage_idx, symbol in enumerate(symbols):
        print(f"\n{'='*70}\n[stage28-dual-sol-bnb-v1] STAGE {stage_idx+1}/{len(symbols)}: {symbol}\n{'='*70}", flush=True)
        ...
        # 1. ehlers-ec-ema-cross
        stage_cells.extend(run_ehlers_ec_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 2. vervoort-zlha-typ-cross
        stage_cells.extend(run_vervoort_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 3. ehlers-fir-zl-price-cross
        stage_cells.extend(run_fir_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 4. dv2-varadi-midline
        stage_cells.extend(run_dv2_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))
        ...
        print(
            f"\n[stage28-dual-sol-bnb-v1] {symbol} STAGE SUMMARY: "
        ...
        if not active_keys and stage_idx < len(symbols) - 1:
            print(
                f"[stage28-dual-sol-bnb-v1] No strategy passed {symbol} 6m gate. "
