"""stage30-dual-sol-bnb-v1 harness: Sweep and score strategies (BTC -> ETH -> SOL -> BNB).
...
Design Bias:
  BTC LEAD PRIMARY CRITICAL — clear past DVS 1.093x / FVE 1.055x / Vervoort 0.832x to >= 1.20 without over-damp.
...
from backtest.path_b.stage30_dual_sol_bnb_v1 import (
    DEFAULT_SYMBOLS,
    RESEARCH_ID,
    STRATEGY_IDS,
)
...
# ---------------------------------------------------------------------------
# Strategy 1: blau-dti-zero-cross
# ---------------------------------------------------------------------------

def run_blau_dti_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "blau-dti-zero-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, BlauDtiParams]] = [
        ("mode_a|(q2,r20,s5,u3)", BlauDtiParams(mode="mode_a", q=2, r=20, s=5, u=3)),
        ("mode_a|(q1,r10,s3,u1)", BlauDtiParams(mode="mode_a", q=1, r=10, s=3, u=1)),
        ("mode_a|(q3,r32,s5,u3)", BlauDtiParams(mode="mode_a", q=3, r=32, s=5, u=3)),
        ("mode_b|(q2,r20,s5,u3,rising)", BlauDtiParams(mode="mode_b", q=2, r=20, s=5, u=3, rising_req=True)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_dti_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_dti_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_dti_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_dti_bnb_smoke(p, tf)
            ...
            try:
                buys, sells, stops = dti_signals(bars, p)
...
# ---------------------------------------------------------------------------
# Strategy 2: arms-vama-dual-cross
# ---------------------------------------------------------------------------

def run_arms_vama_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "arms-vama-dual-cross"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, ArmsVamaParams]] = [
        ("mode_a|(f8,s55,sn100)", ArmsVamaParams(mode="mode_a", fast_len=8, slow_len=55, sample_n=100)),
        ("mode_a|(f5,s34,sn50)", ArmsVamaParams(mode="mode_a", fast_len=5, slow_len=34, sample_n=50)),
        ("mode_a|(f13,s89,sn200)", ArmsVamaParams(mode="mode_a", fast_len=13, slow_len=89, sample_n=200)),
        ("mode_b|(f8,s55,sn100,trend_close)", ArmsVamaParams(mode="mode_b", fast_len=8, slow_len=55, sample_n=100, trend_req=True)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_vama_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_vama_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_vama_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_vama_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = vama_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Strategy 3: apirine-ma-bands-break
# ---------------------------------------------------------------------------

def run_apirine_ma_bands_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "apirine-ma-bands-break"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, ApirineMaBandsParams]] = [
        ("mode_a|(p1_50,p2_10,mltp1.0)", ApirineMaBandsParams(mode="mode_a", p1=50, p2=10, mltp=1.0)),
        ("mode_a|(p1_34,p2_8,mltp0.75)", ApirineMaBandsParams(mode="mode_a", p1=34, p2=8, mltp=0.75)),
        ("mode_a|(p1_100,p2_20,mltp1.5)", ApirineMaBandsParams(mode="mode_a", p1=100, p2=20, mltp=1.5)),
        ("mode_b|(p1_50,p2_10,mltp1.0,narrow1.5)", ApirineMaBandsParams(mode="mode_b", p1=50, p2=10, mltp=1.0, narrow_thr=1.5)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_mab_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_mab_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_mab_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_mab_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = mab_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
# ---------------------------------------------------------------------------
# Strategy 4: ehlers-recursive-median-osc-zero
# ---------------------------------------------------------------------------

def run_ehlers_rmo_cells(
    symbol: str,
    bars_by_tf: dict[str, list[Bar]],
    active_keys: set[str] | None = None,
    prior_results: dict[str, CellResult] | None = None,
) -> list[CellResult]:
    sid = "ehlers-recursive-median-osc-zero"
    results: list[CellResult] = []
    tfs = ("1h", "4h")

    param_grid: list[tuple[str, EhlersRmoParams]] = [
        ("mode_a|(lp12,hp30,med5)", EhlersRmoParams(mode="mode_a", lp=12, hp=30, med_len=5)),
        ("mode_a|(lp8,hp20,med5)", EhlersRmoParams(mode="mode_a", lp=8, hp=20, med_len=5)),
        ("mode_a|(lp16,hp40,med5)", EhlersRmoParams(mode="mode_a", lp=16, hp=40, med_len=5)),
        ("mode_b|(lp12,hp30,med5,rising)", EhlersRmoParams(mode="mode_b", lp=12, hp=30, med_len=5, rising_req=True)),
    ]
    ...
    for tf in tfs:
        ...
        for desc, p in param_grid:
            ...
            btc_ok, btc_msg = validate_rmo_btc_smoke(p, tf)
            eth_ok, eth_msg = validate_rmo_eth_smoke(p, tf)
            sol_ok, sol_msg = validate_rmo_sol_smoke(p, tf)
            bnb_ok, bnb_msg = validate_rmo_bnb_smoke(p, tf)
            btc_smoke_str = "Y" if btc_ok else f"FAIL: {btc_msg}"
            eth_smoke_str = "Y" if eth_ok else f"FAIL: {eth_msg}"
            sol_smoke_str = "Y" if sol_ok else f"FAIL: {sol_msg}"
            bnb_smoke_str = "Y" if bnb_ok else f"FAIL: {bnb_msg}"
            ...
            try:
                buys, sells, stops = rmo_signals(bars, p)
                metrics, tiny_k, thin_f = _eval_windows(symbol, sid, bars, buys, sells, stops)
                cell.metrics = metrics
                cell.tiny_n_kill = tiny_k
                cell.thin_n_flag = thin_f
...
def run_stage30_dual_sol_bnb_v1(
    ...
    for stage_idx, symbol in enumerate(symbols):
        print(f"\n{'='*70}\n[stage30-dual-sol-bnb-v1] STAGE {stage_idx+1}/{len(symbols)}: {symbol}\n{'='*70}", flush=True)
        ...
        # 1. blau-dti-zero-cross
        stage_cells.extend(run_blau_dti_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 2. arms-vama-dual-cross
        stage_cells.extend(run_arms_vama_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 3. apirine-ma-bands-break
        stage_cells.extend(run_apirine_ma_bands_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))

        # 4. ehlers-recursive-median-osc-zero
        stage_cells.extend(run_ehlers_rmo_cells(symbol, bars_by_tf, active_keys, prior_results_by_key))
        ...
        print(
            f"\n[stage30-dual-sol-bnb-v1] {symbol} STAGE SUMMARY: "
        ...
        if not active_keys and stage_idx < len(symbols) - 1:
            print(
                f"[stage30-dual-sol-bnb-v1] No strategy passed {symbol} 6m gate. "
...
def write_scoreboard(results: list[CellResult]) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        f"  1. `blau-dti-zero-cross`",
        f"  2. `arms-vama-dual-cross`",
        f"  3. `apirine-ma-bands-break`",
        f"  4. `ehlers-recursive-median-osc-zero`",
        "",
        "## Executive Summary",
