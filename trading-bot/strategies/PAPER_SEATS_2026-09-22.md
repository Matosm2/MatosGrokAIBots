...
**Status:** PAPER SEATS ONLY (LIVE OFF). Authorized by CoS/Nuno under the 2026-09-22 Path B Mode-A per-coin census.  
**Path B Research Status:** HARD STOP — no stage32 or new Path B packs encoded.

---

## 1. Executive Summary & Authorized Paper Seats

The 2026-09-22 Mode-A per-coin census confirmed:
...
- **Authorized seats:** Exactly two seats qualified across all 30 stages:
  1. **BTCUSDT Seat:** `bostian-iii-sma-zero` @ **4h** (`smaLen=21`) — **1.412× B&H**, **n=40** (Stage 17, PR#45)
  2. **ETHUSDT Seat:** `accdist-sma-cross-v1` @ **4h** (`smaLen=50`) — **1.850× B&H**, **n=46** (Stage 1, PR#29)
...
| **Primary Param** | `smaLen = 21` | `smaLen = 50` |
| **Entry Condition** | `ta.crossover(iiiS, 0)` AND `cooldownOk` | `ta.crossover(adl, adlSma)` AND `cooldownOk` |
| **Exit Condition** | `ta.crossunder(iiiS, 0)` AND `strategy.position_size > 0` | `ta.crossunder(adl, adlSma)` AND `strategy.position_size > 0` |
| **Buy `qty_pct`** | **2.5** | **2.5** |
...
| **Hard Stop** | None (signal exits only; live needs later stop brief) | None (signal exits only; live needs later stop brief) |
...
## 5. LIVE OFF Safety Verification Checklist
...
- [ ] **Stop Brief Gate:** Both strategies operate without hard stops (signal exit only). Real LIVE execution is strictly gated pending a future stop brief.
- [ ] **Path B Freeze:** Research freeze in effect. No Stage 32 or new exploration packs encoded.
